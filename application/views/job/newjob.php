...
<div id="job_contents">
  <div class="job_center">
    <div class="company_wrap">
      ...
      <p style="margin:10px 0px;"><strong> 회사기업： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->company_nature;?>
      </p>
      <p style="margin:10px 0px;"><strong> 회사분류： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->company_profession;?>
      </p>
      <p style="margin:10px 0px;"><strong> 회사규모： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->company_size;?>
      </p>
      <p style="margin:10px 0px;"><strong> 회사위치： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->area;?>
      </p>
      <p style="margin:10px 0px;"><strong> 상세주소： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->company_address;?>
      </p>
      <p style="margin:10px 0px;"><strong> 회사사이트： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->company_site;?>
      </p>
      <p style="margin:10px 0px;"><strong> 담당자： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->responsible_user;?>
      </p>
      <p style="margin:10px 0px;"><strong> 연계전화： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->telphone;?>
      </p>
      <p style="margin:10px 0px;"><strong> 이메일： </strong>
        <?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->company_email;?>
      </p>
    </div>
    <div class="company_wrap">
 <?=form_open($this->uri->uri_string(),array('id'=>'form_job')) ?>
        <input name="company" value="<?php if ($cp_details->num_rows() > 0) $row = $cp_details->row();echo $row->id;?>" type="hidden">
        <table>
          <tbody>
            <tr>
              <td colspan="4">
