...
class Job extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->data['css'] = 1;
		$this->data['cssfile'] = 'job.css';
	}
 
 function index()
 {
		$this->load->library('pager'); 
		$list = $this->pager->init('m23_job',30)->ar();
		//取得地址上的第三个参数
		$current = $this->uri->segment(3); 
		$this->data['joblist'] = $list->query;
		$this->data['current'] = $current;
		
		$query = $this->db->get_where('m23_job', array('is_notice' => 1));
		$this->data['job_notice'] = $query;
 ...
 function newjob()
 {
		...
		if($this->input->get("com_id")){ 
			$query = $this->db->get_where('m23_company', array('id' =>$this->input->get("com_id")));
			if($query->num_rows() > 0)
			 {
				$this->data['cp_details'] = $query;
			//직장 리스트
			//$query = $this->db->get_where('m23_job_comments', array('comment_post_ID' => $id));
			//$this->data['comment_list'] = $query;
			...
			$query = $this->db->get_where('m23_company', array('id' =>$this->input->post('company')));
	 		$row = $query->row();
				...
				$sql = array(
				   ...
				   'iscompany' => 1,
				   'company_name' => $query->row()->corporate_name,
...
// Produces: INSERT INTO mytable (title, name, date) VALUES ('{$title}', '{$name}', '{$date}')
		    ...
		    {
				$this->load->view('job/position',$this->data);
		    }
		}
		else
		{
...
if ($this->fx_auth->is_logged_in()) {									// logged in
	...
	$query = $this->db->get_where('m23_company', array('author' => $this->fx_auth->get_username()));
	if($query->num_rows() > 0)
	 {
		$this->data['cp_details'] = $query;
	//직장 리스트
	//$query = $this->db->get_where('m23_job_comments', array('comment_post_ID' => $id));
	//$this->data['comment_list'] = $query;
		$this->load->view('job/company',$this->data);
	  }
			$this->form_validation->set_rules('corporate_name', 'Title', 'required|xss_clean');
			$this->form_validation->set_rules('nature', 'nature', 'required|xss_clean');
			...
			$this->form_validation->set_rules('addr_prv', 'addr_prv', 'required|xss_clean');
			$this->form_validation->set_rules('intro', 'intro', 'required|xss_clean');
			$this->form_validation->set_rules('contacts', 'contacts', 'required|xss_clean');
			...
			if ($this->form_validation->run())
		    {
				...
				$sql = array(
				   ...
				   'company_profession' =>  $this->input->post('industry'),
				   'company_size' =>  $this->input->post('size'),
				   'area' =>  $this->input->post('addr_prv'),
				   'company_address' =>  $this->input->post('addr_street'),
				   'company_content' =>  $this->input->post('intro'),
				   'responsible_user' =>  $this->input->post('contacts'),
				   'telphone' => $this->input->post('tel'),
				   'company_email' =>  $this->input->post('email'),
				   'creat_time' =>  date('Y-m-d H:i:s')
            	);

				$this->db->insert('m23_company', $sql); 
				$this->load->view('job/company',$this->data);
		    }
		    else
		    {
 ...
 {
		if ($this->fx_auth->is_logged_in()) {									// logged in
			$this->load->view('job/message',$this->data);
		}
		else
		{
			redirect('/auth/login/');
