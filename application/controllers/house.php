...
class House extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->data['css'] = 1;
		$this->data['cssfile'] = 'house.css';
	}
 
 function index()
 {
		$this->load->library('pager'); 
		$list = $this->pager->init('m23_house',30)->ar();
		//取得地址上的第三个参数
		...
		$this->data['error'] = '';
  		...
  		$this->load->view('house/index',$this->data);
 }
 ...
 function newform()
 {
	 	if ($this->fx_auth->is_logged_in()) {									// logged in
			...
			$this->load->view('house/new',$this->data);
		}
		else
		{
			redirect('/auth/login/');
 ...
 function new_submit()
 {
	 if ($this->fx_auth->is_logged_in()) {									// logged in
	  ...
	  {
	   $error = array('error' => $this->upload->display_errors(),'css'=>'1','cssfile'=>'house.css','user_status'=>$this->fx_auth->is_logged_in(),'username'=>$this->fx_auth->get_username());
	   $this->load->view('house/index', $error);
	  } 
	  else
	  {
	   $data = array('upload_data' => $this->upload->data(),'css'=>'1','cssfile'=>'house.css','user_status'=>$this->fx_auth->is_logged_in(),'error'=>'','username'=>$this->fx_auth->get_username());
